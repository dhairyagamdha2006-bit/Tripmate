...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!request) {
      return jsonError('Trip request not found.', 404);
    }
    return jsonOk(mapTripDetail({ trip, request, booking: trip.bookings[0] ?? null }));
  } catch (error) {
    return jsonError(error instanceof Error ? error.message : 'Unable to load trip.', 400);
  }
}
